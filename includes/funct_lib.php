...
define ("LINKS_INTRO_FIELDS", "(`paragraph`, `text`, `page_id`)");
define ("LINKS_CONTRIBUTOR_FIELDS", "(`name`, `title`, `location`, `email`, `url1`, `url1_title`, `page_id`)");
define ("LINKS_RECORDER_FIELDS", "(`name`, `address`, `telephone`, `fax`, `email`, `page_id` )");
define ("LINKS_SPECIES_FIELDS", "(`title`, `text`, `page_id`)");
define ("LINKS_LINKS_FIELDS", "(`title`, `url1`, `url1_title`, `link_desc`, `page_id`)");
define ("LINKS_LINKS2_FIELDS", "(`title`, `url1`, `url1_title`, `url2`, `url2_title`, `text`, `page_id`)");
define ("LINKS_MAILING_FIELDS", "(`title`, `url1`, `url1_title`, `post_email`, `contact`, `sub_email`, `unsub_email`, `sub_message`, `body_message`, `text`, `page_id`)");
define ("LINKS_PHOTO_FIELDS", "(`country`, `filename`, `width`, `height`, `text`)");
define ("LINKS_TOPSITES_FIELDS", "(`title`, `grid_reference`, `text`, `page_id`)");
define ("LINKS_USEFUL_FIELDS", "(`title`, `text`, `isbn`, `page_id`)");
define ("LINKS_USEFULINFO_FIELDS", "(`title`, `text`, `page_id`)");
define ("LINKS_MAP_FIELDS", "(`imagemap`, `regionlist`, `page_id`)");
define ("LINKS_BANNERS_FIELDS", "(`filename`, `url1`, `url1_title`, `sponsor`, `text`, `width`, `height`, `page_id`)");
define ("LINKS_SPONSOR_FIELDS", "(`title`, `url1`, `url1_title`, `link_desc`, `filename`, `section`)");
define ("SUBLINKS", "(`sublinks`, `page_id`)");
define ("PAGENAME", "(`title`, `sub-category`, `category`)");
...
//        echo 'cat:  '.$category.'    subcat:  '.$subcategory;
        $text = "";
    ...
    switch ($table) {
        ...
        case "links_introduction":
            if ($category == "news" || $category == "reviews" || $category == "announcements" || $category == "offers" || $category == "feedback") {
                ...
                connect_db();
                $select = "SELECT * FROM `introduction` WHERE `page_id` = (SELECT `page_id` FROM `pagename` WHERE `category` = '$category' AND `sub-category` = '$subcategory') ORDER BY id ASC";
                $result = mysql_query($select);
                $count = mysql_num_rows($result);
                $resultsection = new SectionContent;
                if ($count > 0) {
                    ...
                    $select = "SELECT * FROM `introduction` WHERE `page_id` = (SELECT `page_id` FROM `pagename` WHERE `category` = '$category' AND `sub-category` = 'default_intro') ORDER BY id ASC";
                    //echo $select;
                    $result = mysql_query($select);
                    $count = mysql_num_rows($result);
                    while ($row = mysql_fetch_assoc($result)) {
        ...
        case "links_contributor":
            $select = "SELECT * FROM $table WHERE `page_id` = (SELECT `page_id` FROM `pagename` WHERE `category` = '$category' AND `sub-category` = '$subcategory') ORDER BY id DESC";
            $result = mysql_query($select);
            $count = mysql_num_rows($result);
            if ($count > 0) {
                $resultsection = new SectionContent;
...
class Map extends ConvertInput
{
    ...
    private function createMapCode()
        {
            $subCategory = "";
            $sql = "SELECT `sub-category` FROM `pagename` WHERE `page_id` = " . $this->data[0]['page_id'];
            ...
            if ($this->data[0]['imagefilename'] <> "") {
                $this->mapcode = '<img usemap="#' . $subCategory . '" src="' . SITE_ROOT . MAP_DIR . $this->data[0]['imagefilename'] . '" />';
            }
        }
    ...
    private function getData()
    {
        ...
        if ($this->id <> "") {
            ...
            $sql = "SELECT * FROM $this->table WHERE `page_id` = (SELECT `page_id` FROM `pagename` WHERE `category` = '$this->cat' AND `sub-category` = '$this->subcat') ORDER BY id DESC";
