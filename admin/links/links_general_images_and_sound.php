...
{
    switch ($db_name) {
        case"art_and_artists":
            $db_name = "Bird Art/Artists";
            break;
        case"bird_song":
            $db_name = "Bird Song";
            break;
        ...
        case"webcams":
            ...
            break;
        case"photos":
            $db_name = "Photographs/Photographers";
            break;
        default:
    ...
    <script language="JavaScript">
        ...
        function load(object) {
            if (object.options[object.selectedIndex].value == 'Select a sub-category') {
                ...
                parent.home.location.href = 'pick_record.php?category=images_and_sound&subcategory=' + object.options[object.selectedIndex].value + '&random=1114683798' + rand(255);
                return false;
            }
        }

        function quickload(place) {
            parent.home.location.href = 'pick_record.php?category=images_and_sound&subcategory=' + place + '&random=1114683798' + rand(255);
        }

        function showlinks() {
            if (subcategory == 'Bird Art/Artists') {
                opt('', 'No further response required');
                quickload('art_and_artists');
            }
            if (subcategory == 'Bird Song') {
                opt('', 'No further response required');
                quickload('bird_song');
            }
            if (subcategory == 'Digiscoping') {
                opt('', 'No further response required');
                quickload('digiscoping');
            }
            if (subcategory == 'Webcams') {
                opt('', 'No further response required');
                quickload('webcams');
            }
            if (subcategory == 'Photographs/Photographers') {
                opt('', 'No further response required');
                quickload('photos');
            }
...
<table border="0" cellpadding="0" cellspacing="5">
    <tr>
        <td valign="top"><font face="Verdana,Arial" size="4">&nbsp;Images and sound></font></td>
        <td>
            <form name="category">
                <select name="subcategory" onChange="return reshow(document.category.subcategory)">
                    <option selected>Select a category</option>
                    <?php
                    $category = "images_and_sound";
