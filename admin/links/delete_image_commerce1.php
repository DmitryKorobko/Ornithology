...
if ($action) {
    switch ($action) {
        ...
        case "deletemap":
            //do sql delete
            $sqlsel = "DELETE FROM $info_type";
            $sqlsel .= " WHERE `sub-category` ='" . $subcategory . "'";
            $sqlquery = $sqlsel;
            //echo $sqlquery;
            $result = mysql_query($sqlquery);
            if ($result) {
...
switch ($mode) {
                ...
                while ($row = mysql_fetch_assoc($result)) {
                    ...
                    <tr>
                        ...
                        <td><font face="Verdana,Arial" size="1"><img
                                    src="../../photos/<?php echo html_entity_decode($row["filename"]); ?>"></font></td>
                    </tr>
                    <tr>
    ...
    case "MAP":
        ...
        <p><font face="Verdana,Arial" size="5" color="#FAD400">Delete map</font></p>

        <form name="add_row" action="<?php echo $_SERVER['PHP_SELF']; ?>?action=deletemap" method="post"
            ...
            <table cellpadding="3">
                ...
                <?php
                //get a summary of the recoed that has just been added
                $sqlsel = "SELECT * FROM $info_type WHERE `sub-category`='" . $subcategory . "'";
                //echo $sqlsel;
                $result = mysql_query($sqlsel);
                while ($row = mysql_fetch_assoc($result)) {
                    ?>
