...
{
    switch ($db_name) {
        case"big_days_and_bird_racing":
            $db_name = "Big days and bird racing";
            break;
        case"big_sits":
            $db_name = "Big sits";
            break;
    ...
    <script language="JavaScript">
        ...
        function load(object) {
            if (object.options[object.selectedIndex].value == 'Select a sub-category') {
                ...
                parent.home.location.href = 'pick_record.php?category=listing_and_racing&subcategory=' + object.options[object.selectedIndex].value + '&random=1114684424' + rand(255);
                return false;
            }
        }

        function quickload(place) {
            parent.home.location.href = 'pick_record.php?category=listing_and_racing&subcategory=' + place + '&random=1114684424' + rand(255);
        }

        function showlinks() {
            if (subcategory == 'Big days and bird racing') {
                opt('', 'No further response required');
                quickload('big_days_and_bird_racing');
            }
            if (subcategory == 'Big sits') {
                opt('', 'No further response required');
                quickload('big_sits');
            }
            if (subcategory == 'Listing') {
                opt('', 'No further response required');
                quickload('listing');
            }
            if (subcategory == 'Twitching') {
                opt('', 'No further response required');
                quickload('twitching');
            }
            if (subcategory == 'Web twitching') {
                opt('', 'No further response required');
                quickload('web_twitching');
            }
...
<table border="0" cellpadding="0" cellspacing="5">
    <tr>
        <td valign="top"><font face="Verdana,Arial" size="4">&nbsp;Listing & racing></font></td>
        <td>
            <form name="category">
                <select name="subcategory" onChange="return reshow(document.category.subcategory)">
                    <option selected>Select a category</option>
                    <?php
                    $category = "listing_and_racing";
