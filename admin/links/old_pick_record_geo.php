...
<p>
    ...
    if ($subcategory == "georgia") {
        $photosubcat = $category . $subcategory;
    } else {
        $photosubcat = $subcategory;
    }
    $select = "SELECT * FROM $photo WHERE `country` = '$photosubcat'";
    // echo $select.' ';
...
<p>
    ...
    <img src="<?php echo ROOT_URL; ?>/adverts/<?php echo $row["filename"]; ?>" width="<?php echo $row["width"]; ?>"
         height="<?php echo $row["height"]; ?>" border="0"><br>
    <font face="verdana" size="1" color="#808080"><?php echo auto_link(encode($row["url1"])); ?></font><br>
    <font face="verdana" size="1" color="#808080"><?php echo auto_link(encode($row["url1_title"])); ?></font><br>
...
<?php
$select = "SELECT *, date_format(timestamp, '%d/%m/%Y') as timestamp FROM $blogs WHERE `category` = '$category' AND `sub-category` = '$subcategory' ORDER BY title";
//echo $select;
$result = mysql_query($select);
$count = mysql_num_rows($result);
