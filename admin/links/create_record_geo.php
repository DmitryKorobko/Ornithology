...
<p>
    ...
    $select = "SELECT * FROM $photo WHERE `country` = '$country'";
    //echo $select;
    $result = mysql_query($select);
    ...
    if ($count > 0) {
        while ($row = mysql_fetch_assoc($result)) {
            ?>
            <img src="../../photos/<?php echo $row["filename"]; ?>" width="<?php echo $row["width"]; ?>"
...
echo $map;
$select = "SELECT * FROM map WHERE `page_id` = '$pageId'";
//echo $select;
$result = mysql_query($select);
$count = mysql_num_rows($result);
...
if (($count > 0) || ($map <> "")) {
    while ($row = mysql_fetch_assoc($result)) {
        echo "<p>" . html_entity_decode($row["imagemap"]) . "</p>";
        echo "<p>" . html_entity_decode($row["regionlist"]) . "</p>";
    }
    ?>
