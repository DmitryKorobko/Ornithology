...
$sqlLinks = "SELECT `page_id` FROM `links`";
$result2 = mysql_query($sqlLinks);
while ($row = mysql_fetch_assoc($result2)) {
    $ids .= $row['page_id'] . ',';
}
$sqlsel = "SELECT DISTINCT `category`, `sub-category` FROM `pagename` WHERE `page_id` IN (" . $ids . ") ORDER BY `category`, `sub-category`";
...
$sqlLinks = "SELECT `page_id` FROM `links_useful_information`";
$result2 = mysql_query($sqlLinks);
while ($row = mysql_fetch_assoc($result2)) {
    $ids .= $row['page_id'] . ',';
}
...
//get a list of all categorys and subcategorys which will include all geo links pages
$sqlLinks = "SELECT `page_id` FROM `links_reserves`";
$result2 = mysql_query($sqlLinks);
while ($row = mysql_fetch_assoc($result2)) {
    $ids .= $row['page_id'] . ',';
}
