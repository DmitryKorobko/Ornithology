...
// for iweb mysql server
	define ("DB_HOST", "localhost");
	define ("DB", "fbiwadm_fatbirdernet");
	...
	*/
	...
	define ("chmod_ip", "67.205.102.4");
	// define ("chmod_ip", "ftp.fatbirder.net");
	define ("chmod_login", "fbiwadm");
	define ("chmod_pass", "H0rnb1ll");
	...
	switch ($table){
		...
		//echo $select."<br>";
				...
				// include default trip report link
					$content .= "<p><b>Travelling Birder</b><br>";
					$content .= "<small><a href=\"http://www.travellingbirder.com/tripreports/default.php\" target=\"_blank\">http://www.travellingbirder.com</a></small><br>";
					$content .= "The <b>Travellingbirder.com</b> birding trip report search engine guides you to 7,000+ birding trip reports on the Internet. You can search for trip reports from a specific country and time of year. Not all these reports are in English. So, if you can�t find the trip report you want on this Fatbirder page� give them a try!</p>";
				...
				*/
				...
				$count = mysql_num_rows($result);
				if ($count>0){
					if($table == "links_trip_reports"){
						$content .= '<a class="content_link" href="http://www.birderstravel.com" target="_blank"><img src="/images/bt_logo_on_fb_advert_blue.gif" width="235" height="80" border="0"></a>';
					}		
					while ($row = mysql_fetch_assoc($result)){
						$content .= "<p>";
						if ($row["title"]){
		...
		//echo $select."<br>";
				...
				if ($count>0)
				{
					...
					$content .= "</p><p>";
				    if ($row["url"]){
					$content .= "<a href=".$row["url"]." target=\"_blank\"><img class=\"banner_link\" src=\"".ROOT_URL."/adverts/".$row["filename"]."\" width=\"".$row["width"]."\" height=\"".$row["height"]."\" border=\"0\" align=\"left\" hspace=\"10\"></a>";
					}
					if ($row["text"]){
	   ...
	   case "links_observatories":
		...
		case "links_useful_reading":
			...
			$result = mysql_query($select);
				$count = mysql_num_rows($result);
				if ($count>0){
					$content .= '<a class="content_link" href="http://www.nhbs.com/index.html?ad_id=5" target="_blank"><img src="/images/gen_title_nhbs.gif" border="0" height="80" width="235"></a>';
					while ($row = mysql_fetch_assoc($result)){
						$content .= "<p>";
						if ($row["title"]){
						$content .= "<h2>".auto_link( htmlentities($row["title"]))."</h2>";
