...
class BirdOrder
{
  ...
  /*  protected function getData($orderby = "", $order = "", $limit = "")
    ...
    public function addRecord($params)
    {
        $dbcon = new Dbconnection();
        $db = $dbcon->connect();
        $sql = 'INSERT INTO `' . $this->table . '` (`scientificname`, `commonname`, `displayorder`, `page_id`) VALUES (?, ?, ?, ?)';
        $result = $db->prepare($sql);
    ...
    public function updateRecord($params)
    {
        $dbcon = new Dbconnection();
        $db = $dbcon->connect();
        $sql = 'UPDATE `' . $this->table . '` SET `scientificname`=?, `commonname`=?, `displayorder`=? WHERE `id`=?';
        $result = $db->prepare($sql);
        $result->execute($params);
        $no = $result->rowCount();
        if ($no == 0) {
