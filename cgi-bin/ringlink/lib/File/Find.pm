...
# that allow for taint mode with Perl 5.005_03:
# - 5.005_64 requirement commented out
# - 'our' replaced with 'use vars'
# - untainting enabled by default
#
# Ringlink CVS ID:
# $Id: Find.pm,v 1.2 2003/06/12 23:53:48 gunnarh Exp $
#
...
sub _find_opt {
    ...
    $untaint          = 1;                        # $wanted->{untaint}
    $untaint_pat      = qr|^([-+@\w./: \\]+)$|;   # $wanted->{untaint_pattern}
    $untaint_skip     = $wanted->{untaint_skip};

    # for compatability reasons (find.pl, find2perl)
    use vars qw($topdir $topdev $topino $topmode $topnlink);
