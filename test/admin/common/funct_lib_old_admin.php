...
define ("ROOT_URL", "http://www.fatbirder.com");
	...
	define ("chmod_ip", "77.73.4.57");
	define ("chmod_login", "fbiwadm");
	define ("chmod_pass", "F1r3cr0wn");
	define ("chmod_file", "/www/");
 // set or alter dir_const based on the absolute dir of a web site location if php installation is part of apache
 // if php is running as a cgi then set to "../../" instead
...
/*
	...
	if (!$conn_id) {
		print "<br>Unable to ftp to $ip<br>";
	} // else print "<br>ftp to $ip sucess $conn_id<br>";
	$login_result=@ftp_login($conn_id, $login, $pass);
	if (!$login_result) {
		print "<br>Unable to ftp login with $login - $pass<br>";
	} // else print "<br>ftp login with $login - $pass sucess $login_result<br>";
	$chmod_cmd="chmod ".$chmod_mode." ".$file;
	$chmod=ftp_site($conn_id, $chmod_cmd);
//	$chmod = ftp_chmod($conn_id,$chmod_mode,$file);
	if (!$chmod) {
		print "<br>Unable to execute ftp cmd $chmod_cmd<br>";
	}
	if ($chmod == 1){
...
*/
	function ftp_change_mod($ip, $login, $pass, $file, $chmod_mode){
		$conn_id=ftp_connect($ip);
		$login_result=@ftp_login($conn_id, $login, $pass);
	/*	if (ftp_chmod($conn_id, $chmod_mode, $file) !== false) {
			//echo "ftp chmod of '$file' changed to $chmod_mode<br>";
		} else {
			echo "failed to change chmod of '$file' to $chmod_mode<br>";
		}
	*/
		...
		if ($chmod == 1){
			...
			echo "<br>failed to change chmod of '$file' to $chmod_mode<br>";
		}
...
function createHTML($tmpl_dir="", $tmpl_name, $html_dir="./", $html_name, $category, $subcategory, $stream=0, $article_id=""){
	...
	//	echo "<br>rootfir $rootdir<br>";
		...
		// echo "<br>template path ".$template_path;
		...
		if(!$template_handle) {
				...
				********* DEAL WITH TEMPLATE TOKENS *********/
				...
				while ($tok) {
					...
					case "TITLE":
						//echo $tok;
						//$table = "map";
						$string = gettitle($subcategory, $category)."<BR>";
						$output_string .= html_entity_decode($string);
					break;
					case "ADSLEFT":
						$target = dir_const."adimages/adsleft.inc.php";
						$output_string .= file_get_contents($target);
					break;
					case "ADSRIGHTTOP":
						$target = dir_const."adimages/adsrighttop.inc.php";
						$output_string .= file_get_contents($target);
					break;
					case "ADSRIGHTBOTTOM":
						$target = dir_const."adimages/adsrightbottom.inc.php";
						$output_string .= file_get_contents($target);
					break;
					case "ADDTHIS":
						$target = dir_const."admin/common/includes/addthis.inc.php";
						$output_string .= file_get_contents($target);
					break;
					case "CROSSLINK":
						//echo $tok;
						/*list ($alt_dir, $alt_file) = makeALTDIR($category, $subcategory);
						$link = '/'.$alt_dir.$alt_file;
						$string = getcrosslink('fatfisherman.com', $link);
						$output_string .= html_entity_decode($string);
						$string = getcrosslink('fatphotographer.net', $link);
						$output_string .= html_entity_decode($string);*/
					break;
					case "PHOTO":
						$table = "photos";
				...
				******* DISPLAY SECTION HEAD FOOT ***********/
				...
				for ($i=0; $i< count($matches[0]); $i++) {
				  ...
				  break;
				  case "FAMILYLINKSHEADER":
					$header = $matches[0][$i];
					if ($familylinkscheck <> 1){
					$output_string = str_replace($header, "", $output_string);
					}
				  break;
				  case "FAMILYLINKSFOOTER":
					$footer = $matches[0][$i];
					if ($familylinkscheck <> 1){
					$output_string = str_replace($footer, "", $output_string);
					}
				  break;
				  case "SPECIESLINKSHEADER":
					$header = $matches[0][$i];
					if ($specieslinkscheck <> 1){
					$output_string = str_replace($header, "", $output_string);
					}
				  break;
				  case "SPECIESLINKSFOOTER":
					$footer = $matches[0][$i];
					if ($specieslinkscheck <> 1){
					$output_string = str_replace($footer, "", $output_string);
					}
			   ...
			   if ($stream==0){
				   if ($output_string) {
					   // print "<br>filedir $file_dir <br>filename $html_name";
				   ...
				   // try and change an existing file to 777
				   // if it don't exist not a prob as it will be owned by nobody.
				   // if it does exist it may be owned by fbiwadm so we need to 777 it
				   if (!$BuildStatic = fopen($file_path, "w")) print "<br>cannot open $file_path for write";
				   if (!fputs($BuildStatic,$output_string)) print "<br>failed to put file";
				   fclose($BuildStatic);
	...
	switch ($table){
      ...
      case "links_blogs":
		//echo $select."<br>";
				...
				// include default trip report link
					$content .= "<p><b>CloudBirders</b><br>";
					$content .= "<small><a href=\"http://www.cloudbirders.com\" target=\"_blank\">Trip Report Repository</a></small><br>";
					$content .= "CloudBirders was created by a group of Belgian world birding enthusiasts and went live on 21st of March 2013. They provide a large and growing database of birding trip reports, complemented with extensive search, voting and statistical features�</p>";
				}
				if ($table == "links_holiday_companies"){
					...
					if ($count>0)
					{
						while ($row = mysql_fetch_assoc($result))
						{
							...
							if ($row["url1"]) {
								$content .= "<small>".auto_link( htmlentities($row["url1"]), $row['url1_title'])."</small><br>";
							}
							if ($row["link_desc"]){
				...
				*/
				...
				$count = mysql_num_rows($result);
				if ($count>0){
					if($table == "links_trip_reports" || $table == "links_holiday_companies"){
						$content .= '<div class="content_link"><p>This section sponsored by: <a class="content_link" href="http://www.birderstravel.com" target="_blank"><img src="/images/sponsorbirderstravel.jpg" width="200" height="60" border="0"></a></p></div>';
					}
					while ($row = mysql_fetch_assoc($result)){
						$content .= "<p>";
						if ($row["title"]){
		...
		//echo $select."<br>";
				...
				if ($count>0)
				{
					while ($row = mysql_fetch_assoc($result))
					{
						// hack to allow youtube embedded objects without auto_link stuffing it up
						$gotyt = strpos($row["text"],"youtube");
						if ($gotyt !== false) {
							//$content .= "<p>".htmlentities($row["text"])."<!-- chris --></p>";
							$content .= "<p>".$row["text"]."<!-- chris --></p>";
						} else {
							$content .= "<p>".auto_link( htmlentities($row["text"]))."<!-- auto linked --></p>";
						}
					}
				}else{
		...
		//echo $select."<br>";
				...
				if ($count>0)
				{
					...
					{
					$content .= "<p>";
					$content .= "<small>This page is sponsored by...</small><br>";
					if ($row["sponsor"]){
						// hack to stop auto_link stuffing up fully formed pre-entered urls
						$gotyt = strpos($row["sponsor"],"youtube");
						if ($gotyt !== false) {
							$content .= "<p>".$row["sponsor"]."</b>";
						} else {
					...
					$content .= "</p><p>";
				    if ($row["url1"]){
					$content .= "<a href=".$row["url1"]." target=\"_blank\"><img class=\"banner_link\" src=\"".ROOT_URL."/adverts/".$row["filename"]."\" width=\"".$row["width"]."\" height=\"".$row["height"]."\" border=\"0\" align=\"left\" hspace=\"10\"></a>";
					}
					if ($row["text"]){
					$content .= auto_link( htmlentities($row["text"]));
					}
					$content .= "<br clear=\"all\">&nbsp;</p>";
					$content .= "<p>&nbsp;</p>";
					}
				}
		break;
				...
				if ($count>0)
				{
					while ($row = mysql_fetch_assoc($result))
					{
					   ...
					   if ($row["url1"]) {
						...
						if ($row["url2"]){
						   $content .= "<small>".auto_link( htmlentities($row["url2"]), $row['url2_title'])."</small><br>";
						}
						if ($row["text"]){
							$content .= auto_link( htmlentities($row["text"]));
						}
		...
		//echo $select."<br>";
				...
				if ($count>0)
				{
					while ($row = mysql_fetch_assoc($result))
					{
					  ...
					  $content .= "<img src=\"".ROOT_URL."/images_news/";
						  if($row["image"]){
						  $content .= $row["image"];
						  }else{
						  $content .= "news1.gif";
		...
		//echo $select."<br>";
				...
				if ($count>0)
				{
					while ($row = mysql_fetch_assoc($result))
					{
					  ...
					  $content .= "<img src=\"".ROOT_URL."/images_reviews/";
						  if($row["image"]){
						  $content .= $row["image"];
						  }else{
	...
	switch($category){
		case "ornithology":
			...
			case "europe":
				switch($subcategory){
					case "germany_deutschland_baden-w�rttemberg":
						$alt_dir = "links_geo/$category/";
					...
					break;
/*
// no need for these scotish alt page names now as of 30/07/09 - cmn
...
function auto_link($link, $title=null) {
	...
	if (is_null($title) || trim($title) == ''){
		...
		$link_repl = '<a href="\1" target="_blank">'.$title.'</a>';
		$email_repl = '<a href="mailto:\1\2\3">'.$title.'</a>';
	}
	$link = preg_replace($link_reg, $link_repl, $link);
	$link = preg_replace($email_reg, $email_repl, $link);
...
function getcrosslink ($domain,$pageref){
$link = 'http://www.'.$domain.$pageref;
echo '<br>xlink: '.$link.'<br>';
$fasttest=1;
    //return 'ok';
    $main = array();
    $ch = curl_init();
    ...
    if (preg_match('/HTTP\/1\.\d+\s+(\d+)/', $response, $matches))
    {
        ...
        if ($ok)
        {
			return '<BR><SPAN class="crosslink"><a href="'.$link.'" target="blank">(visit this page on '.$domain.')</a></SPAN>';
        }
        else
        {
			return "<BR>";
