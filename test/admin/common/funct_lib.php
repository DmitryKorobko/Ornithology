...
require(ROOT_DIR . "admin/firelogger.php");
	// db settings
...
define ("USER", "root");
define ("PASS", "X5h7W23g");*/
define ("ROOT_URL", "http://fatbirder.loc");
define ("dir_const", "/home/steinmann/www/fbiwadm/www/");
...
class ConvertInput{
	...
	private function prepareUpload($icon = FALSE){
		$maxwidth = $width;
		$maxheight = $height;
	   ...
	   case "links_observatories":
		...
		case "NEWS":
			if($category == "default_home"){
				...
				if ($count>0){
					$resultsection = new SectionContent;
					$content .= $resultsection->sectionStart("News");
					while ($row = mysql_fetch_assoc($result)){
						$content .= $resultsection->newsitemStart();
						if($row["image"]){
							$content .= $resultsection->newsIcon($row["image"], "news");
						}else{
						   $content .= $resultsection->newsIcon("news1.gif", "news");
						}
						$content .= $resultsection->newsItemTitle("news", $row["id"], $row["title"]);
						$content .= $resultsection->newsItemPreview($row["summary"]);
						$content .= $resultsection->newsItemDate($row["created"]);
						$content .= $resultsection->newsitemEnd();
					}
					$content .= $resultsection->newsArchiveLink("news");
					$content .= $resultsection->sectionend();
				}
			}
		break;
...
class SectionContent{
	...
	public function intro($text, $defhome){
		if($defhome == "default_home"){
			$sectioncontent .= decode($text);
		}else{
			$sectioncontent .= '<p class="text">' . decode($text) . '</p>';
		}
	...
	public function newsItemSection($section){
		$sectioncontent .= '<p class="newsitemtype">' . decode($section) . '</p>';
		return $sectioncontent;
	}

	public function newsItemTitle($dir, $id, $title){
		$sectioncontent .= '<h3 class ="newsitemtitle"><a href="' . $dir . '/index.php?article=' . $id . '">' . decode($title) . "</a></h3>";
		return $sectioncontent;
	}
	public function newsItemPreview($summ){
		$sectioncontent .= '<p class="newsitempreview">' . decode($summ) . '</p>';
		return $sectioncontent;
	}
	public function newsItemDate($newstime){
		$sectioncontent .= '<p class="newsitemdate">' . $newstime . '</p>';
		return $sectioncontent;
	}
	...
	public function title($titletext){
		$sectionoutput = '<p class="resultarea_item_title">' . decode($titletext) . '</p>';
		return $sectionoutput;
	}

	public function endemicsTitle($entitle){
		$sectioncontent .= '<p><span class="bold">Number of endemics: ' . decode($entitle) . '</p>';
		return $sectioncontent;
	}

	public function speciesTitle($sptitle){
		$sectioncontent .= '<p><span class="bold">Number of bird species: ' . decode($sptitle) . '</p>';
		return $sectioncontent;
	}
	...
	public function text($anytext){
		$sectionoutput = '<p class="resultarea_item_text">' . decode($anytext) . '</p>';
		return $sectionoutput;
	}
