...
class Banner extends prepareInput
{
    ...
    private function getDataByPageId($cat, $subCat){//retrieve - where pageId is already known
        $sql = "SELECT * FROM $this->table WHERE `category` = '$cat' AND `sub-category` = '$subCat'";//TODO: Change category an subcategory to pageId
        $con = new Dbconnection();
        $db = $con->connect();
        $result = $db->prepare($sql);
        $result->execute();
    ...
    {
        $sql = "INSERT INTO $this->table (`filename`, `width`, `height`, `alt`, `url1`, `url1title`, `sponsor`, `bannertext`, `category`, `sub-category`) VALUES (:filename, :width, :height, :alt, :url1, :url1title, :sponsor, :bannertext, :category, :subcategory)";
        $dbcon = new Dbconnection();
        $db = $dbcon->connect();
        $result = $db->prepare($sql);
        $result->execute($this->params);
